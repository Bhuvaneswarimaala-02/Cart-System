...
def add_to_cart(user_id, item_name, quantity):
    ...
    if price == 0:
        print(f"⚠️ DEBUG: Price is 0 for {item_name}, check products collection!")
...
def get_price(item_name):
    ...
    if not product:
        print(f"❌ DEBUG: Product '{item_name}' not found in products collection!")
        return 0

    if "price" not in product:
        print(f"⚠️ DEBUG: 'price' field missing for {item_name}")
        return 0

    print(f"✅ DEBUG: Fetched price ₹{product['price']} for {item_name}")
...
def update_cart_item(user_id, item_name, new_quantity):
    ...
    if existing_item:
        ...
        cart_collection.update_one(
            {"user_id": user_id, "item_name": existing_item["item_name"]},  # Exact name from DB
            {"$set": {"quantity": new_quantity, "price": new_price}}  # ✅ Ensure price is updated
        )
        return f"✅ Updated {existing_item['item_name']} to {new_quantity} units at ₹{price_per_unit} each."
